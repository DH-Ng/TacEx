...
@configclass
class BallRollingHeightMapEnvCfg(DirectRLEnvCfg):
    # viewer settings
    ...
    # curriculum settings
    curriculum_steps = [2e7]  # after this amount of common_steps (= total steps), we make the task more difficult
    ...
    observation_space = {
        "proprio_obs": (
            14
        ),  # 3 for ee pos, 2 for orient (roll, pitch), 2 for goal (x,y) and 2 for obj-pos (x,y), 5 for actions
        "vision_obs": [32, 32, 1],
    }
...
class BallRollingHeightMapEnv(DirectRLEnv):
    ...
    cfg: BallRollingHeightMapEnvCfg

    def __init__(self, cfg: BallRollingHeightMapEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        reaching_penalty = torch.where(
            object_ee_distance > self.cfg.too_far_away_threshold - 0.05, reaching_penalty + 10, reaching_penalty
        )
