...
from .factory_env_modified_obs_cfg import (
    OBS_DIM_CFG,
    STATE_DIM_CFG,
    FactoryEnvModifiedObsCfg,
)

from tacex_sim2real.franky_isaac_robot import FrankyIsaacRobotCfg, FrankyIsaacRobot


class FactoryModifiedObsEnv(DirectRLEnv):
    cfg: FactoryEnvModifiedObsCfg

    def __init__(
        self, cfg: FactoryEnvModifiedObsCfg, render_mode: str | None = None, **kwargs
    ):
        ...
        self._panda_hand_idx = body_ids[0]
        ...
        self._jacobi_panda_hand_idx = self._panda_hand_idx - 1
    ...
    def _init_tensors(self):
        ...
        self.fingertip_body_idx = self._panda_hand_idx
    ...
    def _setup_scene(self):
        ...
        if hasattr(self.cfg.robot, "isaac_articulation_cfg"):
            self._robot = FrankyIsaacRobot(self.cfg.robot)
        else:
    ...
    def _get_factory_obs_state_dict(self):
        ...
        state_dict = {
            "fingertip_pos": self.fingertip_midpoint_pos,
            "fingertip_pos_rel_fixed": self.fingertip_midpoint_pos
            - self.fixed_pos_obs_frame,
            "fingertip_quat": self.fingertip_midpoint_quat,
    ...
    def generate_ctrl_signals(
        ...
        # Franky wrapper only works with joint position targets, so we compute desired joint target
    ...
    def randomize_initial_state(self, env_ids):
        ...
        fixed_asset_pos_rand = torch.tensor(
            self.cfg.obs_rand.fixed_asset_pos, dtype=torch.float32, device=self.device
        )
        ...
        if self.cfg_task.name == "gear_mesh":
            fixed_tip_pos_local[:, 0] = (
                self.cfg_task.fixed_asset_cfg.medium_gear_base_offset[0]
            )
    ...
    def _compute_frame_pose(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        ee_pos_w = self._robot.data.body_pos_w[:, self._panda_hand_idx]
        ee_quat_w = self._robot.data.body_quat_w[:, self._panda_hand_idx]
